...
try {
  ...
  content = content.replace(/\/flashcard-app\/\//g, '/flashcard-app/');

  // Fix double base paths (flashcard-app/flashcard-app -> flashcard-app)
  content = content.replace(
    /\/flashcard-app\/flashcard-app\//g,
    '/flashcard-app/'
  );

  // Ensure all asset paths start with the correct base path (only for paths that don't already have it)
  content = content.replace(
    /href="\/(?!flashcard-app\/)([^"]*)/g,
    'href="/flashcard-app/$1'
  );
  content = content.replace(
    /src="\/(?!flashcard-app\/)([^"]*)/g,
    'src="/flashcard-app/$1'
  ...
  const notFoundContent = fixedIndexContent.replace(
    '</body>',
    redirectScript + '\n  </body>'
  );
